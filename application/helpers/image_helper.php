...
function uploadFile_start()
{
    if (!empty($_FILES)) {
        //target
        $targetDir = "uploads/fileuploads_dv_temp/";
        ...
        if(imagefn()->db->insert("files_dv_temp" , $arsaveFileData))
        {
            echo json_encode(
                [
                    "status" => "success", 
...
function removeFile_start()
{
    ...
    if (!empty($data->fileName)) {
        ...
        imagefn()->db->where("f_name" , $data->fileName);
        imagefn()->db->delete("files_dv_temp");
